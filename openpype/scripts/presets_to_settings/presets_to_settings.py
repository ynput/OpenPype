...
def convert_global_anatomy(
    ...
    # --- ImageIO ---
    # TODO this is not done (NOT SURE ABOUT MAPPING of imageio keys)
    return

    imageio_entity = anatomy_entity["imageio"]

    presets_init = presets_data.get("init") or {}
    color_init = presets_init.get("colorspace")
    color_data = presets_data.get("colorspace", {}).get(color_init)

    dataflow_init = presets_init.get("dataflow")
    dataflow_data = presets_data.get("dataflow", {}).get(dataflow_init)

    color_nuke_data = {}
    color_hiero_data = {}
    if color_data:
        color_nuke_data = color_data.get("nuke") or {}
        color_hiero_data = color_data.get("hiero") or {}

    nuke_imageio_entity = imageio_entity["nuke"]
    # NUKE presets
    {
        "root": {
        ...
        "write": {
            "render": {"colorspace": "linear"},
            "prerender": {"colorspace": "linear"},
