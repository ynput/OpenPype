...
class CollectMantraROPRenderProducts(pyblish.api.InstancePlugin):
    ...
    def process(self, instance):
        ...
        for index in range(1, aov_numbers + 1):
            var_ = rop.evalParm("vm_variable_plane{}".format(index))

            # skip empty variables
            if not var_:
                continue

            aov_name = "vm_filename_plane{}".format(index)
            aov_boolean = "vm_usefile_plane{}".format(index)
            ...
            files_by_aov[var_] = file_collection.generate_expected_filepaths(
