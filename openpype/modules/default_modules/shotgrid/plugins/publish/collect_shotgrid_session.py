...
class CollectShotgridSession(pyblish.api.ContextPlugin):
    ...
    def process(self, context):
        ...
        self.log.info("shotgrid_servers_settings: {}".format(shotgrid_servers_settings))

        shotgrid_server = shotgrid_settings.get('shotgrid_server', "")
        if not shotgrid_server:
            self.log.error("No Shotgrid server found, please choose a credential"\
        ...
        if not shotgrid_script_name and not shotgrid_script_key:
            self.log.error("No Shotgrid api credential found, please enter "\
