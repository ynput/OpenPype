...
if sys.__stdout__:
    ...
    def _print(message: str, force: bool = False):
...
def _determine_mongodb() -> str:
    ...
    if not openpype_mongo:
        ...
        if os.getenv("OPENPYPE_HEADLESS_MODE") == "1":
            _print("!!! Cannot open Igniter dialog in headless mode.", True)
            _print(("!!! Please use `OPENPYPE_MONGO` to "
                    "specify server address."), True)
            sys.exit(1)
...
def boot():
    ...
    if getattr(sys, 'frozen', False):
        ...
        if not result[0]:
            _print(f"!!! Invalid version: {result[1]}", False)
            sys.exit(1)
